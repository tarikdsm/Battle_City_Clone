...
- **The flat-graphic path has no vertical probe.** The frame wall's vertical faces measure −34.3% against `boardFrame` — the one place §3.0's "the authored hex appears" promise is not kept. Blast radius is small (the frame is the only vertical flat graphic), but add a vertical flat-graphic probe when convenient.
