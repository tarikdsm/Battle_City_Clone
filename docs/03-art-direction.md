...
2. **The canopy jiggle in "Tree rustle" is not implemented.** Canopies are instanced statics written once by `terrainView.build`, whose whole design is "no per-frame rebuilds" (arch §5), so jiggling one means re-opening a 1 200-line tested view to animate a 150 ms decoration. The three leaf motes ship. Deliberate, not forgotten.
