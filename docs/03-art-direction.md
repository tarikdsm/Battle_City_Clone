...
| Smoke | `#7a808f` | §8's dust puffs, explosion smoke, base column, eagle wisps |
...
*(The smoke row was added 2026-08-02 with T4.1/T4.2, by the rule the pedestal row below established: §8 asks for smoke in five of its twelve rows and §3.1 authored no colour for it, so the doc gains a row rather than the code an unauthored grey. Chosen against its neighbours: **4.4× `board`** in luminance so a puff is visible over the darkest thing it can sit on, and **0.83× `eagleStone`** so it never reads as a piece of the pedestal it drifts off. Saturation 14.7%, i.e. below §6's 55% threshold, so it reads **warm-sided by design** — which is the right outcome for smoke lit by an explosion.)*
...
- **Dynamic point-light pool — ships at 6** (measured, T4.1; the authored "max 8" stands as a ceiling, 6 is the calibrated value). 4 and 6 measure identically; **8 costs +1.1 ms and halves the delivered frame rate** on the reference iGPU, reproduced twice. 7 is untested. Priority by proximity/importance: muzzle flash (range 40 u, 60 ms), bullet glow (tiny, attached, Low: off), explosion (range 90 u, 400 ms, quadratic decay), base explosion (range 160 u, 1.2 s), power-up idle pulse (range 24 u, 1.2 s sine), spawn star (range 30 u).
  **Shipped at 6, not 8 (measured, T4.1/T4.2).** A resident point light costs every lit fragment a shader iteration whether it is on or off, and the eighth is not a linear step but a cliff. One played stage at High, `docs/calibration/fx.json`: **0 lights** (pre-FX) 2.03 ms mean / 3.5 p95 / 28.5 fps · **4** 2.47 / 3.7 / 49.9 · **6** 2.35 / 3.5 / 46.4 · **8** 3.45–3.67 / 5.3–6.1 / 23.5–26.6. Four and six are indistinguishable; eight costs **+1.1 ms of frame CPU and roughly half the delivered frame rate**, reproduced across two runs. "Max 8" is a ceiling, so six is inside this doc; arch §11's first budget row ("60 FPS sustained: desktop @High") is what the eighth light spends. Low halves it to 3. Raise the number back only with a measurement.
  **Intensities are not authorable here and are not in this list.** three 0.185.1 is physically based, so a point light's contribution is `intensity / d²` and the number that reads correctly depends on the scene's scale — this board is 208 **u** across. They live in `FX_LIGHTS` (`src/render/fx/fxSystem.ts`), quoted as `range² × wanted` so each is re-derivable.
  **An idle light must be dark, never hidden.** `WebGLRenderer.projectObject` skips an invisible object, so hiding a light drops `numPointLights`, which is part of `programCacheKey` — every material then recompiles on every flicker. Measured on the first T4.1 capture: mean frame time 2.0 → **11.9 ms** with a **1394 ms** worst frame. Intensity is a uniform; visibility is a program.
...
**Accepted: the FXAA pass emits two shader-compile warnings on ANGLE/D3D.** Measured T3.2 and settled — do not re-investigate. Under ANGLE's D3D11 backend, compiling `three/examples/jsm/shaders/FXAAShader.js` logs `warning X3595: gradient instruction used in a loop with varying iteration` **twice**, and on some driver versions `warning X4000: use of potentially uninitialized variable (f_ApplyFXAA)`. The source is upstream and unambiguous: lines **193** and **211** of that file are `for ( int i = 1; i < EDGE_STEP_COUNT && !pAtEnd; i++ )` — two loops whose trip count is decided at runtime, each sampling a texture inside the body, which is verbatim the condition X3595 describes; two loops is exactly the two warnings. `ApplyFXAA` (line 262) is the enclosing function HLSL inlines as `f_ApplyFXAA`. **High emits zero** — it uses SMAA, not FXAA — so the warnings appear only on Medium and Low. They are recorded per preset in `docs/calibration/play.json` (`shaderWarnings`). Nothing in this repo's own shaders is implicated, and the only fixes available would be vendoring a patched copy of upstream's shader or dropping FXAA; neither is justified by a compile-time warning about derivative precision inside an edge-search loop. The DoD's "no three.js warnings" is read as *no warnings from our own code*.
...
| Event | Recipe | Budget |
|---|---|---|
| Shot fired | muzzle star sprite 60 ms + point light + barrel recoil + **2** spark motes | 3 particles (star + 2 motes; the light and the recoil are not particles) |
...
**Built 2026-08-02 (T4.1/T4.2), and three things this table said turned out to need a ruling.**

1. **The Budget column is the number of pooled particles, and the "Shot fired" row overspends its own.** That row describes a muzzle star sprite *plus* three spark motes — four objects against a stated budget of three. The budget is the hard cap, so the shipped recipe is **one star + two motes**. Either the budget rises to 4 or the prose drops a mote; the code currently honours the number.
2. **The canopy jiggle in "Tree rustle" is dropped** (coordinator ruling, 2026-08-02): canopies are instanced statics written once by `terrainView.build`, whose whole design is "no per-frame rebuilds" (arch §5), and a per-frame write path into them is not worth a 150 ms decorative tell. The row is **three leaf motes and nothing else** — and because concealment is a mechanic whose counter-play is *noticing* it, they were strengthened at T4.3 to carry the tell alone (2.7 u flakes at 1.7–2.1× the canopy tokens, 1.25 s, wide throw).
3. **The base's shockwave is 96/130 u, not the widest a "double ring" suggests.** The eagle sits half a tile from the south edge (fidelity §2), so *any* ring wider than a tile crosses the frame wall and keeps going: a 168 u ring drew a gold arc across the black letterbox **outside** the board. There is no clip; the size is the clip.

**Three rows retuned at T4.3** (coordinator's calls after seeing the T4.2 captures): **Ice skid** is drawn at `iceSheen × 1.4` on a 3.4 × 13.6 u decal with brighter, streaked frost motes — ice is a *gameplay* surface and the player must know they are sliding without inferring it from drift. **Base explosion**'s point light drops from 5200 to **2400**: at 5200 the eagle's own brick nest sat ~16 u from the light, i.e. at 20× the calibrated key, and blew to near-white over the ruined pedestal — which is the game-over signal §4's destroyed eagle exists to carry. **Tree rustle** is above.

**Measured cost of the whole VFX layer** (`docs/calibration/fx.json`, Intel UHD / ANGLE D3D11, 1600×900 DPR 1, one played stage): scene draw calls at High went **43 → 45 median, 45 → 52 max** against this section's own ≤ 60 budget; Low went 16 → 17 median. Art §8's ~180-particle cap costs **5 draws** (one `InstancedMesh` per particle *kind*) plus 2 in the bloom source pass — **+7 at High, +5 at Low** — and an idle board costs nothing at all, because an `InstancedMesh` at `count === 0` issues no draw. Frame CPU at High: 2.03 → 2.38 ms mean, 3.5 → 3.7 p95.
...
- **High-contrast mode is required, not optional** (upgraded 2026-08-02). **The prescription changed on measurement (T6.1) — outlines are out.** Two reasons, both decisive: a tank is a single `InstancedMesh` whose instances are ~20 *parts*, so an inverted-hull outline traces every box rather than the silhouette; and an outline does not move either body's **value**, which is the problem this mode exists to solve. What ships instead: **players wear their §3.1 accent token, every enemy token scales by 0.6, and tracers brighten.**
- **The problem was also understated here by 7×.** This doc cited player-gold vs enemy-gunmetal at ~18 luma points. The genuine worst pair is **P1 gold vs Fast sand at 2.62** (BT.601, 0–255). Measured effect of the shipped mode: worst pair **2.62 → 59.98 (×22.9)**; the previously-cited pair 18.37 → 108.53. Artifact: `docs/calibration/high-contrast.json`. Type silhouettes clear the bar among *enemies*; player-vs-enemy separation currently leans on hue, which is exactly what a colourblind player does not get. This mode is the fallback that makes that acceptable — ship it in Phase 6 with the other settings.
- Effects never cover the player's own tank for >100 ms; smoke max alpha 0.35 over playfield.
- `prefers-reduced-motion` or settings toggle: **suppress every camera motion that carries no gameplay information** — shake, slow-mo, screen flash, **and the stage fly-in** (ratified 2026-08-02, T4.3: a 23° sweep is the canonical vestibular trigger, and the curtain wipe still delivers the beat). All gameplay information is preserved, and effects still emit their particles. The earlier list of three was illustrative, not exhaustive; this principle governs.
- **"Nearby" for §2's explosion trauma** = full strength at 32 u, zero at 64 u, linear between (T4.3; §2 said "nearby" and stopped).
